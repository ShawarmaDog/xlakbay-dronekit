...
try:
    while True:
        ret = park_at_aruco()
        if ret == 0:
            print("Vehicle is within 1 meter of the marker. Stopping movement.")
        time.sleep(.05)
        
except KeyboardInterrupt: #if you ctrl+c the program, it will stop the rover
    print("Program interrupted. Stopping the vehicle.")
    send_local_ned_velocity(0, 0, 0)  # Ensure the vehicle stops
    vehicle.armed = False

'''
#If you'd like the rover to disarm when it is within 1m of the aruco marker
success_counter=0
